...
1. 发起者在小程序里选择一个副本。
2. 点击「发起 1 分钟召集」。
3. 小程序创建一场独立召集，并自动把发起者加入报名池。
4. 发起者点击「分享到群」。
5. 群友点开分享卡片，输入自己的邀请码后参与报名。
6. 倒计时结束后，后端从所有报名者里随机抽出副本固定人数；如果报名人数少于固定人数，则从现有报名者中抽出全部可选玩家。
7. 被抽中的玩家会写入 `match_room_players`，后续可作为一次副本参与记录。

一次副本限制：

- 副本标记为 `is_one_shot = true` 后，被抽中过的玩家不能再次发起或报名该副本。
